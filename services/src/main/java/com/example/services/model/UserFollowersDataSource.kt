...
class UserFollowersDataSource(var screenName : String) : PageKeyedDataSource<String, UserInfo>() {
    ...
    override fun loadInitial(params: LoadInitialParams<String>, callback: LoadInitialCallback<String, UserInfo>) {
        ...
        var authorizationHeader = CreateHeaderService.createHeader(parameterValues,"GET","1.1/followers/list.json")
        disposable = twitterapiService.getFollowersOfUser(authorizationHeader,screenName)
            .subscribeOn(Schedulers.io())
            .observeOn(AndroidSchedulers.mainThread())
            .subscribe ({ result ->
                callback.onResult(result.follwerList,null,result.nextCursor)
            },{
                    error -> Log.e("Datasource for follower", error.message)
            })
    }
    ...
    override fun loadAfter(params: LoadParams<String>, callback: LoadCallback<String, UserInfo>) {
        ...
        disposable = twitterapiService.getFollowersOfUser(authorizationHeader,screenName,params.key)
            ...
            .observeOn(AndroidSchedulers.mainThread())
            .subscribe ({ result ->
                callback.onResult(result.follwerList,result.nextCursor)
            },{
                    error -> Log.e("Datasource for follower", error.message)
            })
    }
